...
function PostResponse({ postId }) {
    ...
    return (<div className="post-responses">
        ...
        {responses.length === 0 ? ('') : (responses.map((response) => (
            <div key={response._id || Math.random()} className="card mb-3">  <div className="card-body">
                <div className="responseData"> <p className="card-text">  {response.content && response.content.trim()
                ...
                <div className="buttons">
                    ...
                    <button
                        className={`btn ${response.liked ? 'btn-success' : 'btn-outline-primary'} like-button`}
                        onClick={() => handleToggleLike(postId, response._id, response.liked)}
                    >
                        <i className="bi bi-hand-thumbs-up"></i> ({response.likesCount || 0})
                    </button>
